...
abstract class DirectRequest extends Request
{
  ...
  public function send(array $parameters = [])
  {
    ...
    $this->config->get('paybox.notifications.enabled') &&
      ...
      $reference = !empty($parameters[DirectQuestionField::REFERENCE])
        ? $parameters[DirectQuestionField::REFERENCE]
        : false;
      ...
      if ($reference) {
        ...
        $job = new NotifyPaymentStatus(
          Notification::createFromResponse(
            $response->getModel(),
            $reference,
            $amount
          ),
   ...
   * @return array
   */
  abstract public function getBasicParameters();
